...
function constructor($scope,
	...
	$scope.selecionaProduto = function () {
		if ($scope.produto.selecionado) {
			$scope.produto.selecionado.desconto = $scope.produto.selecionado.descontoSugerido;
	...
	$scope.calculaTotalPedido = function () {
		var total = 0;
		PedidoService.pedido.itensPedido.forEach(function (item) {
			total = total + (item.precoFinal * item.quantidadeSolicitada);
		});
		return total;
	}

	/*Calcula o total de descontos do pedido*/
	$scope.calculaTotalDescontoPedido = function () {
		var total = 0;
		PedidoService.pedido.itensPedido.forEach(function (item) {
			if(item.desconto > 0) {
				total += (item.desconto * item.preco) * item.quantidadeSolicitada;
			}
		});
		return total;
	}

	/*Calcula o total de acrescimos do pedido*/
	$scope.calculaTotalAcrescimosPedido = function () {
		var total = 0;
		PedidoService.pedido.itensPedido.forEach(function (item) {
			if(item.desconto < 0) {
				total += item.desconto * item.preco * -1 * item.quantidadeSolicitada;
			}
		});
		return total;
	}
